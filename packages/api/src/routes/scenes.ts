...
): { imageModels: string[]; animationModels: string[]; imagesPerScene: number; clipsPerScene: number; imageAspectRatio: string; imageResolutions?: Record<string, string>; clipOptions?: ClipGenerationOptions } {
    ...
    (async () => {
      await Promise.all(
        imageRecords.map(async (imageRecord) => {
          try {
            const result = await generateImage(imageRecord.model, imageRecord.prompt, gen.imageAspectRatio, gen.imageResolutions?.[imageRecord.model]);
